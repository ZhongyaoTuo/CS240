	...
	)
{
  ...
  torch::Tensor out_depth = torch::full({3, H, W}, 0.0, float_opts);
...
//   return std::make_tuple(rendered, out_color, out_depth, radii, geomBuffer, binningBuffer, imgBuffer);
  return std::make_tuple(rendered, out_color, out_depth, radii, geomBuffer, binningBuffer, imgBuffer, pixel_gaussian_counter);
}
...
void setExpandFactor(float expand_factor) {
	CudaRasterizer::Rasterizer::setExpandFactor(expand_factor);
}