...
renderCUDA(
	...
	const float* __restrict__ dL_avgdepths,
	float3* __restrict__ dL_dmean2D,
	float4* __restrict__ dL_dconic2D,
	float* __restrict__ dL_dopacity,
	float* __restrict__ dL_dcolors,
	float* __restrict__ dL_dinvdepths,
	float* __restrict__ dL_davgdepths
)
{
	...
	float accum_avgdepth_rec = 0;
	if (inside)
	{
		for (int i = 0; i < C; i++)
			dL_dpixel[i] = dL_dpixels[i * H * W + pix_id];
		dL_avgdepth = dL_avgdepths[pix_id];
		if(dL_invdepths)
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if(dL_davgdepths) {
			accum_avgdepth_rec = last_alpha * last_avgdepth + (1.f - last_alpha) * accum_avgdepth_rec;
			last_avgdepth = collected_depths[j];
			dL_dalpha += (last_avgdepth - accum_avgdepth_rec) * dL_avgdepth;
			atomicAdd(&(dL_davgdepths[global_id]), dchannel_dcolor * dL_avgdepth);
			}
