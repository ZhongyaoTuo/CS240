...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, batch_size=None, additional_context=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb, batch_size=batch_size)
            elif isinstance(layer, SpatialTransformer):
                x = layer(x, context, additional_context=additional_context)
...
class ResBlock(TimestepBlock):
    ...
    def _forward(self, x, emb, batch_size=None):
        ...
        # print(f"[DEBUG] : ResBlock : _forward end: h {h.shape}")
        return h
...
class UNetModel(nn.Module):
    ...
    def __init__(self,
        ...
        print("[DEBUG] : UNetModel : len(self.input_blocks) = ", len(self.input_blocks))
    ...
    def forward(self, x, timesteps, context=None, features_adapter=None, fs=None, camera_pose=None, **kwargs):
        b,_,t,_,_ = x.shape
        ...
        for id, module in enumerate(self.output_blocks):
            ...
            if self.use_guidance is not None:
                if self.use_guidance == "adapter":
                    if ((id+1)%3 == 0) and features_adapter is not None:
                        additional_context = features_adapter[-1 * (id // 3) - 1]
            # ------------------------------- #
            h = module(h, emb, context=context, batch_size=b, additional_context=additional_context)
        h = h.type(x.dtype)
        y = self.out(h)
