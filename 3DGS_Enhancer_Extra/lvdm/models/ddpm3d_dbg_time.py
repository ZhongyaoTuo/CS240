...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        # CUSTOM ------------------------------------------------------------ #
        self.only_train_addon = only_train_addon
...
class LatentDiffusion(DDPM):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with self.ema_scope():
            ...
            loss_dict_ema = {key + '_ema': loss_dict_ema[key] for key in loss_dict_ema}
        print("[validation] loss_dict_no_ema:", loss_dict_no_ema)
        self.log_dict(loss_dict_no_ema, prog_bar=False, logger=True, on_step=False, on_epoch=True)
        self.log_dict(loss_dict_ema, prog_bar=False, logger=True, on_step=False, on_epoch=True)
...
class LatentCustomDiffusion(LatentDiffusion):
    def __init__(self, img_cond_stage_config, image_proj_stage_config, freeze_embedder=True, image_proj_model_trainable=True, 
                 cond_select_all=False, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.cond_select_all = cond_select_all
        self.image_proj_model_trainable = image_proj_model_trainable
        self._init_embedder(img_cond_stage_config, freeze_embedder)
        self._init_img_ctx_projector(image_proj_stage_config, image_proj_model_trainable)
        self.use_guidance = self.model.diffusion_model.use_guidance
        if self.only_train_addon:
            # freeze U-Net except guidance model
            for param in self.model.parameters():
                if param.name != 'guidance_model':
                    param.requires_grad = False
            print("[INFO] : freeze U-Net & only train adapter")
    ...
    def shared_step(self, batch, random_uncond, **kwargs):
        x, c, fs, camera_pose = self.get_batch_input(batch, random_uncond=random_uncond, return_fs=True)
        kwargs.update({"fs": fs.long()})
        
        _DEBUG = True # HARDCODED
        ...
        if _DEBUG:
            end_event.record()
            torch.cuda.synchronize()
            print(f"[DEBUG] : shared_step time = {start_event.elapsed_time(end_event)} ms")
    ...
    def get_batch_input(self, batch, random_uncond, return_first_stage_outputs=False, return_original_cond=False, return_fs=False, return_cond_frame=False, return_original_input=False, **kwargs):
        ...
        z = self.encode_first_stage(x) # HERE
        
        if _DEBUG:
            end_event.record()
            torch.cuda.synchronize()
            print(f"[DEBUG] : encode_first_stage time = {start_event.elapsed_time(end_event)} ms")
        ...
        z_render = self.encode_first_stage(render) # HERE
        
        if _DEBUG:
            end_event.record()
            torch.cuda.synchronize()
            print(f"[DEBUG] : encode_first_stage (2) time = {start_event.elapsed_time(end_event)} ms")
        ...
        if isinstance(cond_input, dict) or isinstance(cond_input, list):
            cond_emb = self.get_learned_conditioning(cond_input) # HERE
        else:
            cond_emb = self.get_learned_conditioning(cond_input.to(self.device)) # HERE
        
        if _DEBUG:
            end_event.record()
            torch.cuda.synchronize()
            print(f"[DEBUG] : get_learned_conditioning time = {start_event.elapsed_time(end_event)} ms")
        ...
        img_emb = self.embedder(img) ## b l c # HERE
        
        if _DEBUG:
            end_event.record()
            torch.cuda.synchronize()
            print(f"[DEBUG] : embedder time = {start_event.elapsed_time(end_event)} ms")
    ...
    def encode_guidance(self, guidances):
        return self.model.diffusion_model.guidance_model(guidances)
    ...
    def log_images(self, batch, sample=True, ddim_steps=50, ddim_eta=1., plot_denoise_rows=False, \
        ...
        z, c, xrec, xc, fs, cond_x, camera_pose = self.get_batch_input(batch, random_uncond=False,
                                                return_first_stage_outputs=True,
                                                return_original_cond=True,
                                                return_fs=True,
                                                return_cond_frame=True)
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        ...
        # print("[DEBUG] : self.diffusion_model : ", self.diffusion_model)
        # exit()

    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None,
                c_adm=None, s=None, mask=None, c_guidance=None, **kwargs):
        ...
        elif self.conditioning_key == 'hybrid-adapter':
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            if "features_adapter" in kwargs.keys():
                c_guidance = kwargs["features_adapter"]
                del kwargs["features_adapter"]
            out = self.diffusion_model(xc, t, context=cc, features_adapter=c_guidance, **kwargs)
        # elif self.conditioning_key == 'hybrid-gating-attn':
        #     xc = torch.cat([x] + c_concat, dim=1)
        #     cc = torch.cat(c_crossattn, 1)
        #     out = self.diffusion_model(xc, t, context=cc, features_adapter=c_guidance, **kwargs)
