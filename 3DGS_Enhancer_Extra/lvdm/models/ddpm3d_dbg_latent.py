...
class LatentCustomDiffusion(LatentDiffusion):
    ...
    def get_batch_input(self, batch, random_uncond, return_first_stage_outputs=False, return_original_cond=False, return_fs=False, return_cond_frame=False, return_original_input=False, **kwargs):
        ...
        # x = super().get_input(batch, self.first_stage_key)
        ## encode video frames x to z via a 2D encoder
        if self.first_stage_key != 'video_z':
            x = super().get_input(batch, self.first_stage_key)
            z = self.encode_first_stage(x)
        else:
            # x = super().get_input(batch, 'video')
            print("[DEBUG] : use video_z as latent")
            z = super().get_input(batch, 'video_z')
            x = z
        ...
        if self.first_stage_key != 'video_z':
            ...
            print("[DEBUG] : use renders_z as latent")
            z_render = super().get_input(batch, 'renders_z')
            # z_render = rearrange(z_render, 'b t c h w -> b c t h w')
        
        print("[DEBUG] : z.shape = ", z.shape, "z_render.shape = ", z_render.shape)
        ## get caption condition
        cond_input = batch[self.cond_stage_key]
