...
def calculate_psnr(gt: torch.Tensor, pred: torch.Tensor) -> float:
    """
    Calculate PSNR for a batch of images.

    Args:
        gt (torch.Tensor): Ground truth images, shape (N, 3, H, W), in [0,1].
        pred (torch.Tensor): Predicted images, shape (N, 3, H, W), in [0,1].

    Returns:
        float: Average PSNR over the batch.
    """
    # Mean-squared error across all samples in the batch
    mse = F.mse_loss(pred, gt, reduction='mean')
    # PSNR = 10 * log10(1 / MSE)  (assuming data in [0, 1])
    psnr = 10 * torch.log10(1.0 / mse)
    return psnr.item()


def calculate_ssim(gt: torch.Tensor, pred: torch.Tensor) -> float:
    """
    Calculate SSIM for a batch of images using pytorch-msssim.
    ...
    """
    # By default, ssim() calculates mean SSIM across the spatial dimensions
    # We then average across the batch dimension manually if needed
    # The 'data_range=1.0' assumes input in [0, 1].
    ssim_val = ssim(pred, gt, data_range=1.0, size_average=True)
    return ssim_val.item()


def calculate_lpips(gt: torch.Tensor, pred: torch.Tensor, net: str = 'vgg') -> float:
    """
    Calculate LPIPS for a batch of images using the lpips library.

    Args:
        gt (torch.Tensor): Ground truth images, shape (N, 3, H, W), in [0,1].
        pred (torch.Tensor): Predicted images, shape (N, 3, H, W), in [0,1].
        net (str): Which network to use for LPIPS. Common choices: 'alex', 'vgg', 'squeeze'.

    Returns:
        float: Average LPIPS distance over the batch.
    """
    # Initialize LPIPS model (can be done outside this function if called repeatedly)
    lpips_model = lpips.LPIPS(net=net)

    # LPIPS models expect images in the range [-1, 1]
    # gt_norm = (gt * 2) - 1
    # pred_norm = (pred * 2) - 1

    # Compute the LPIPS distance for each pair in the batch
    print(pred.shape, gt.shape) # torch.Size([1, 3, 16, 320, 512]) torch.Size([1, 3, 16, 320, 512])
    with torch.no_grad():
        lpips_scores = lpips_model(pred, gt)  # shape (N, 1, 1, 1)

    # Average across the batch
    mean_lpips = lpips_scores.mean()
    return mean_lpips.item()
...
def log_local(batch_logs, save_dir, filename, save_fps=10, rescale=True):
    ...
    pred_batch = batch_logs['samples'] # N frames (B, 3, T, H, W)
    gt_batch = batch_logs['video']  # N frames (B, 3, T, H, W)
    metrics = {'psnr': [], 'ssim': [], 'lpips': []}
    
    for pred, gt in zip(pred_batch, gt_batch):
        pred = pred.permute(1, 0, 2, 3).clamp(-1, 1)
        gt = gt.permute(1, 0, 2, 3).clamp(-1, 1)
        metrics['psnr'].append(calculate_psnr(pred, gt))
        metrics['ssim'].append(calculate_ssim(pred, gt))
        metrics['lpips'].append(calculate_lpips(pred, gt))
    
    metrics['psnr'] = np.mean(metrics['psnr'])
    metrics['ssim'] = np.mean(metrics['ssim'])
    metrics['lpips'] = np.mean(metrics['lpips'])
    ...
    for key in batch_logs:
        ...
        if isinstance(value, list) and isinstance(value[0], str):
            ...
            # torchvision.io.write_video(path, grid, fps=save_fps, video_codec='h264', options={'crf': '10'}) # skip
            ...
            grid = torchvision.utils.make_grid(video_frames, nrow=t//2, padding=0)
            path = os.path.join(save_dir, "%s-%s.jpg"%(filename, key))
            save_img_grid(grid, path, rescale)
            
            # clamepd video_frames
            video_frames = torch.clamp(video_frames, 0., 1.)
            grid = torchvision.utils.make_grid(video_frames, nrow=t//2, padding=0)
            path = os.path.join(save_dir, "%s-%s-clamped.jpg"%(filename, key))
            save_img_grid(grid, path, rescale)
