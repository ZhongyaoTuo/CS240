...
def custom_collate_fn(batch):
    """
    Custom collate function to handle None values in the dataset batch.

    Args:
        batch (list): A list of items returned by the Dataset's __getitem__ method.
                      Some items may be None.
                      
    Returns:
        dict: A collated batch dictionary, excluding None values, with tensor values.
    """
    # Filter out None items
    valid_items = [item for item in batch if item is not None]
    
    if not valid_items:
        # Handle the case where all items are None (return an empty dictionary)
        return None
    
    # Assuming all valid_items are dictionaries, combine them into a single batch
    collated_batch = {}
    for key in valid_items[0]:
        # Convert list of values to tensor
        values = [item[key] for item in valid_items]
        
        # Handle different types of data
        if isinstance(values[0], torch.Tensor):  # Already a tensor
            collated_batch[key] = torch.stack(values)
        else:  # Convert to tensor if not already
