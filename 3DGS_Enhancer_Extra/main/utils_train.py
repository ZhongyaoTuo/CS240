    ...
    if check_config_attribute(model_cfg, "pretrained_checkpoint"):
        ...
        try:
            if 'state_dict' in pl_sd.keys():
                ...
                checkpoint_state_dict = pl_sd["state_dict"]

                # Create a new state_dict to hold updated weights
                updated_state_dict = {}

                # Iterate over the checkpoint's state_dict
                for key in checkpoint_state_dict:
                    if key in model_state_dict:
                        # Load the weight from the checkpoint
                        updated_state_dict[key] = checkpoint_state_dict[key]
                    else:
                        # Optionally, handle keys that are not present in the model
                        print(f"Key '{key}' is not in the model's state_dict; skipping.")
                ...
                # Optionally, verify which keys were missing or unexpected
