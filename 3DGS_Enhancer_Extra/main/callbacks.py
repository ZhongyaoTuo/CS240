...
class ImageLogger(Callback):
    ...
    def log_batch_imgs(self, pl_module, batch, batch_idx, split="train"):
        ...
        # skip_freq = 1 # for debugging
        # if split == "train":
        #     print("[DEBUG] : skip_freq", skip_freq, (batch_idx+1) % skip_freq == 0, split, pl_module.global_rank)

        if split == "train":
            if pl_module.global_step % 3000 == 0:
                skip_freq = 1
            else:
                skip_freq = self.batch_freq
        else:
            skip_freq = 1
        ...
        if (batch_idx+1) % skip_freq == 0:
            ...
            batch_logs['renders'] = batch['renders']
            
            batch_logs = prepare_to_log(batch_logs, self.max_images, self.clamp)
            torch.cuda.empty_cache()
            
            scene_id = batch['scene_id'][0]
            filename = "idx{}_{}".format(
                batch_idx,
                scene_id)
            save_dir = os.path.join(self.save_dir, split, f"{pl_module.current_epoch:03d}")
            if self.to_local:
                mainlogger.info("Log [%s] batch <%s> to local ..."%(split, filename))
                filename = "gs{}_".format(pl_module.global_step) + filename
                metrics = log_local(batch_logs, os.path.join(save_dir, split), filename, save_fps=10)
                pl_module.log_dict(metrics, prog_bar=False, logger=True, on_step=False, on_epoch=True)
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=None):
        ...
        # print("[DEBUG] : pl_module.global_rank", pl_module.global_rank)
...
class TimingLogger(Callback):
    ...
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx, dataloader_idx=None):
        ...
        print("Data loading time: ", self.last_batch_start_time - self.last_batch_end_time)
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=None):
        ...
        batch_time = current_time - self.last_batch_start_time
        self.last_batch_end_time = current_time
        # rank_zero_ info(f"Batch time: {batch_time:.2f} seconds")
