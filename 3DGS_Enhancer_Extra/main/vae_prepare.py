...
if __name__ == "__main__":
    ...
    # print("[INFO] : workdir, ckptdir, cfgdir, loginfo", workdir, ckptdir, cfgdir, loginfo)
    ...
    ## Running LOOP >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # 1. iterate dataset
    # 2. encode video and save .pt file
    ...
    for i, batch in tqdm(enumerate(dataloader)):
        ...
        video = batch['video'] # [1, 3, N, H, W]
        renders = batch['renders'] # [1, 3, N, H, W]
        image_paths = batch['image_paths'] # [1, N]
        render_paths = batch['render_paths'] # [1, N]
        
        video = video.to(model.device)
        renders = renders.to(model.device)
        
        gt_z = torch.empty(1, 4, video.shape[2], 40, 64)
        render_z = torch.empty(1, 4, video.shape[2], 40, 64)
        
        gt_embed = []
        
        pt_path = "/".join(image_paths[0][0].split('/')[:-4])
        print(pt_path)
        
        for i in tqdm(range(0, video.shape[2], chunk)):
            video_chunk = video[:, :, i:i+chunk, :, :]
            renders_chunk = renders[:, :, i:i+chunk, :, :]
            image_paths_chunk = image_paths[i:i+chunk]
            render_paths_chunk = render_paths[i:i+chunk]
            
            # gt_z_chunk = model.encode_first_stage(video_chunk) # [1, 4, chunk, 40, 64]
            # gt_z[:, :, i:i+chunk, :, :] = gt_z_chunk
            
            # # save z per image
            # # for j in range(gt_z_chunk.shape[2]):
            # #     z_per_image = gt_z_chunk[:, :, j, :, :]
            # #     image_path = image_paths_chunk[j][0]
            # #     pt_path = image_path.replace('.png', '_z.pt')
            # #     torch.save(z_per_image, pt_path)

            # del gt_z_chunk
            
            # render_z_chunk = model.encode_first_stage(renders_chunk) # [1, 4, chunk, 40, 64]
            # render_z[:, :, i:i+chunk, :, :] = render_z_chunk
            
            # # for j in range(render_z_chunk.shape[2]):
            # #     render_z_per_image = render_z_chunk[:, :, j, :, :]
            # #     render_path = render_paths_chunk[j][0]
            # #     pt_path = render_path.replace('.png', '_z.pt')
            # #     torch.save(render_z_per_image, pt_path)
            
            # del render_z_chunk  
    
    
            gt_embed_chunk = model.embedder(rearrange(video_chunk, 'b c t h w -> (b t) c h w'))
            gt_embed.append(gt_embed_chunk)
    
        gt_embed = torch.cat(gt_embed, dim=0)
        torch.save(gt_embed, os.path.join(pt_path, 'gt_embed.pt'))
        # torch.save(render_z, os.path.join(pt_path, 'render_z.pt'))
